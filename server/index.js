...
app.use(cors());
app.use(express.json());   // Para que pueda procesar JSON en las peticiones
...
app.post('/api/clientes', (req, res) => {
  ...
  db.query(query, [nombre, email, telefono], (err, results) => {
    ...
    res.status(201).json({ success: true, message: 'Cliente agregado con éxito', id: results.insertId, nombre, email, telefono });
  });
});
...
app.put('/api/clientes/:id', (req, res) => {
  ...
  db.query(query, [nombre, email, telefono, id], (err, results) => {
    ...
    res.json({ success: true, message: 'Cliente actualizado con éxito', id, nombre, email, telefono });
  });
});
